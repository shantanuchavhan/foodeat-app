...
const Page = ({ params }) => {
  ...
  async function removeOrder(orderToRemove) {
    ...
    if (data) {
      setOrders((oldOrders) =>
        oldOrders.filter((order) => order.id !== orderToRemove.id),
      );
              ...
              {order.orderedItems && order.orderedItems.length > 0 ? (
                <div>
                  ...
                  {order.orderedItems.map((item) => (
                    <div
                      key={item.id}
                      className="flex gap-2 justify-between bg-red-300 p-2 "
                    >
                      {/* Render individual ordered item details */}
                      <div>
                        <p className="">{item.menu.itemName}</p>
                        <p className="">Quantity:- {item.quantity}</p>
                        {item.deliveryCompletions == null && (
                          <div>
                            <h1>Arriving soon</h1>
                          </div>
                        )}
                      ...
                      <Popover>
                        <Popover.Button
                          className="px-2 bg-white text-red-400"
                          onClick={() => {
                            setPopoverOpen((old) => true);
                            console.log(
                              "isPopoverOpen Popover.Button:-",
                              isPopoverOpen,
                            );
                          }}
                        >
