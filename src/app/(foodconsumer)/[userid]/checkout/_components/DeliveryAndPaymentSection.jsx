...
const DeliveryAndPaymentSection = ({
  ...
  const [isAddressRequiredSection, setIsAddressRequiredSection] =
    useState(false);
    ...
    <>
      {isAuthRequiredSectionComplete ? (
        <AddresComponent
          ...
          isAddressRequiredSection={isAddressRequiredSection}
        />
      ) : (
      ...
      <>
        {userDetails?.cart?.length > 0 ? (
          ...
          <div>
            <Link
              href={`/`}
              className="bg-green-400 text-center py-5  flex align-center justify-center px-20"
            >
              <h5 className="text-red-600">Grab some foods first</h5>
            </Link>
          </div>
        )}
      </>
