...
const AddNewAdressComponent = () => {
  ...
  const AddAddress = () => {
    setUserDetails((old) => {
      return {
        ...old,
        address: [...old.address, newAddress],
      };
    });
  ...
  return (
    <div className="border  border-solid border-gray p-4 h-12/13 w-3/6 flex flex-col gap-3 align-center justify-center">
        ...
        <div className="relative w-6">
          ...
          <div className="absolute top-0 right-0">
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-3 h-3 rounded-full text-white  bg-green-500"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      <form action={addUserAddress.bind(null, data?.user?.email)}>
        ...
        />
        <button
          className="border  border-solid border-gray  flex align-center justify-center p-3 text-green-500"
          type="submit"
          onClick={AddAddress}
        >
          Add Addresss
